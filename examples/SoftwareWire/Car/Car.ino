...
void setup(){                                                //
    ...
    mot_L.begin(&sWire);                                     //   Инициируем работу с мотором установленным по левому  борту автомобиля, указав ссылку на объект для работы с шиной I2C на которой находится мотор (по умолчанию &Wire).
    mot_R.begin(&sWire);                                     //   Инициируем работу с мотором установленным по правому борту автомобиля, указав ссылку на объект для работы с шиной I2C на которой находится мотор (по умолчанию &Wire).
