...
bool	iarduino_I2C_Motor::_begin			(void){																				//	Параметр: отсутствует
...
bool	iarduino_I2C_Motor::setFreqPWM		(uint16_t frequency){																//	Параметр:				частота в Гц (25-1000).
			if(valAddr){																										//	Если модуль был инициализирован, то ...
				if( frequency<25   ){ return false; }																			//	Если частота указана некорректно, то возвращаем false.
...
bool	iarduino_I2C_Motor::saveManufacturer(uint64_t code){																	//	Параметр:				code - код разработчика.
			if(valAddr){																										//	Если модуль был инициализирован, то ...
			//	Ждём:																											//
				while( (millis()>=timReset) && (millis()<(timReset+150)) ){;}													//	Заперщаем сохранять данные в FLASH в течении 150 мс после перезагрузки (в т.ч. инициализации) модуля.
			//	Готовим данные для передачи:																					//
