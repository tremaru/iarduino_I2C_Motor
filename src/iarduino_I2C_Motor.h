...
#endif																																		//
																																			//
#include		"iarduino_I2C_Motor_I2C.h"																									//	Подключаем библиотеку выбора реализации шины I2C.
																																			//
...
class iarduino_I2C_Motor{																													//
	...
	/**	Пользовательские функции **/																										//
		...
		bool				setPullI2C				(bool=true							);													//	Объявляем  функцию установки    подтяжки шины I2C					(Параметр:  флаг подтяжки).
																																			//
		bool				setFreqPWM				(uint16_t Frequency					);													//	Объявляем  функцию установки    частоты ШИМ							(Параметр:  частота в Гц от 25 до 1000).
																																			//
		bool				setMagnet				(uint8_t Sum						);													//	Объявляем  функцию установки    количества магнитов у датчика Холла	(Параметр:  количество от 1 до 255).
		uint8_t				getMagnet				(void								);													//	Объявляем  функцию возвращающую количество магнитов у датчика Холла	(Параметр:  отсутствует).
																																			//
		bool				setReducer				(float GearRatio					);													//	Объявляем  функцию установки    передаточного отношения редуктора	(Параметр:  отношение от 0.01 до 167'772.15).
		float				getReducer				(void								);													//	Объявляем  функцию возвращающую передаточное  отношение редуктора	(Параметр:  отсутствует).
																																			//
		bool				setError				(uint8_t Percent					);													//	Объявляем  функцию установки    процента макс. отклонения скорости	(Параметр:  процент от 0 до 100 максимально допустимого отклонения реальной скорости от заданной).
		uint8_t				getError				(void								);													//	Объявляем  функцию возвращающую	флаг ошибки скорости или драйвера	(Параметр:  отсутствует).
																																			//
		bool				setSpeed				(float SpeedVal, uint8_t SpeedType, float StopVal, uint8_t StopType);					//	Объявляем  функцию установки    скорости и условия остановки		(Параметры: значение скорости, MOT_RPM / MOT_PWM / MOT_M_S - тип значения скорости, значение условия остановки, MOT_MET / MOT_REV / MOT_SEC - тип условия остановки. При MOT_RPM скорость указывается от 0 до ±32'767 об/мин., при MOT_PWM скорость указывается от 0 до ±100 % ШИМ, при MOT_M_S скорость указывается в м/сек.
		bool				setSpeed				(float SpeedVal, uint8_t SpeedType	){ return setSpeed(SpeedVal,SpeedType,0.00,0xFF);}	//	Определяем функцию установки    скорости							(Параметры: значение скорости, MOT_RPM / MOT_PWM / MOT_M_S - тип значения скорости.                                                                                  При MOT_RPM скорость указывается от 0 до ±32'767 об/мин., при MOT_PWM скорость указывается от 0 до ±100 % ШИМ, при MOT_M_S скорость указывается в м/сек.
		float				getSpeed				(uint8_t SpeedType					);													//	Объявляем  функцию возвращающую реальную скорость или ШИМ			(Параметр:  MOT_RPM / MOT_PWM / MOT_M_S - тип возвращаемого значения).
																																			//
		bool				setStop					(float StopVal, uint8_t StopType	);													//	Объявляем  функцию остановки    мотора по условию					(Параметры: значение условия остановки, MOT_MET / MOT_REV / MOT_SEC - тип условия остановки).
		bool				setStop					(void								){ return setStop(0.00, 0xFF); }					//	Определяем функцию остановки    мотора								(Параметр:  отсутствует).
		float				getStop					(uint8_t StopType					);													//	Объявляем  функцию возвращающую	значение оставшеея до остановки		(Параметр:  MOT_MET / MOT_REV / MOT_SEC - тип возвращаемого значения).
																																			//
		bool				setStopNeutral			(bool Neutral						);													//	Объявляем  функцию установки    нейтрального положения при остановке(Параметр:  флаг true/false).
		bool				getStopNeutral			(void								);													//	Объявляем  функцию возвращающую	флаг нейтраль. пол-ния при остановке(Параметр:  отсутствует).
																																			//
		bool				setDirection			(bool CKW							);													//	Объявляем  функцию установки    направления вращения вала			(Параметр:  флаг вращения вала по ч.с. при положительной скорости).
		bool				getDirection			(void								);													//	Объявляем  функцию возвращающую	направления вращения вала			(Параметр:  отсутствует).
																																			//
		bool				setInvGear				(bool ReducerInv, bool RotorInv		);													//	Объявляем  функцию установки    флагов инверсии механизма			(Параметры: флаг инверсии вращения редуктора, флаг инверсии полярности мотора).
		uint8_t				getInvGear				(void								);													//	Объявляем  функцию возвращающую	байт с флагами инверсии механизма	(Параметр:  отсутствует).
																																			//
		float				getSum					(uint8_t TypeDistance				);													//	Объявляем  функцию возвращающую	пройденный  путь / кол-во оборотов	(Параметр:  MOT_MET / MOT_REV - тип возвращаемого значения).
		bool				delSum					(void								){ return setStop(0.00, MOT_REV); }					//	Определяем функцию сброса		пройденного пути / кол-ва оборотов	(Параметр:  отсутствует).
																																			//
		bool				setVoltage				(float Volts						);													//	Объявляем  функцию установки    номинального напряжения мотора в В	(Параметр:  напряжение в Вольтах).
		float				getVoltage				(void								);													//	Объявляем  функцию возвращающую	номинальное  напряжение мотора в В	(Параметр:  отсутствует).
																																			//
		bool				setNominalRPM			(uint16_t RPM						);													//	Объявляем  функцию установки    номинальной скорости в об/мин.		(Параметр:  скорость вращения вала редуктора от 0 до 65'535 об/мин.).
		uint16_t			getNominalRPM			(void								);													//	Объявляем  функцию возвращающую	номинальную скорость в об/мин.		(Параметр:  отсутствует).
																																			//
		bool				saveManufacturer		(uint64_t Code						);													//	Объявляем  функцию сохранения   конструктивных данных в flash память(Параметр:  код разработчика).
																																			//
	...
	/**	Внутренние переменные **/																											//
		...
		uint32_t			timReset;																										//	Объявляем  переменную для хранения времени последней перезагрузки модуля.
	/**	Внутренние функции **/																												//
